...
export async function recentForContext(
  limit = 30
): Promise<{ angle_tags: string[]; displays: string[]; content_formats: string[] }> {
  const sb = admin();
  ...
  return {
    ...
    content_formats: rows
      .map((r) => {
        const meta = (r as { generation_metadata?: Record<string, unknown> }).generation_metadata;
        const v = meta && typeof meta === "object" ? meta["content_format"] : null;
